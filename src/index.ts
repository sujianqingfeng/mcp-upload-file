...
server.tool('upload-file', 'upload file by url', {
  url: z.string().describe('url'),
  fileName: z.string().describe('filename'),
}, async ({ url, fileName },) => {

  if (!process.env.UPLOAD_URL || !process.env.FILE_KEY || !process.env.FILE_NAME) {
  ...
  if (process.env.EXTRA_FORM) {
    try {
      ...
      Object.entries(extraForm).forEach(([key, value]) => {
